  ...
  async function handleSendMessage(event) {
    ...
    try {
      ...
      setChatMessages(current => [
        ...
        {
          role: 'assistant',
          content: reply || t.chat_error_reply
        }
      ])
    } catch {
      setChatError(t.chat_error_connection)
      setChatMessages(current => [
        ...current,
        {
          role: 'assistant',
          content: t.chat_error_connection
        }
      ])
    } finally {
  ...
  return (
    <div className="floating-buttons-container">
      <a
        ...
        title={t.chat_open_title}
        aria-label={t.chat_open_title}
        onClick={(event) => {
          event.preventDefault()
          openChat()
        }}
      ...
      <a
        ...
        title={t.telegram_button || 'Написать в Telegram'}
        aria-label={t.telegram_button || 'Написать в Telegram'}
      >
      ...
      <a
        ...
        title={t.whatsapp_button || 'Написать в WhatsApp'}
        aria-label={t.whatsapp_button || 'Написать в WhatsApp'}
      >
        ...
        <div className="grok-chat-overlay" role="dialog" aria-modal="true" aria-labelledby="grok-chat-title">
          <div className="grok-chat-panel">
            <div className="grok-chat-header">
              <div>
                <p className="grok-chat-kicker">{t.chat_subtitle}</p>
                <h3 id="grok-chat-title">{t.chat_title}</h3>
              </div>
              <button type="button" className="grok-chat-close" onClick={closeChat} aria-label={t.chat_close}>
                ✕
              </button>
            </div>
            ...
            <form className="grok-chat-composer" onSubmit={handleSendMessage}>
              ...
              <div className="grok-chat-actions">
                <span className="grok-chat-status">{chatError || t.chat_ai_info}</span>
                <button type="submit" disabled={chatLoading || !chatInput.trim()}>
                  {chatLoading ? t.chat_sending : t.chat_send}
                </button>
              </div>
            </form>
          </div>
